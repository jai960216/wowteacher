// ============================================
// /api/wcl/fightPlayerIds — fight 참여자 sourceID 매핑 공유 캐시
// ============================================
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const cacheKey = `fightPlayerIds:${code}:${startTime}:${endTime}`;
  ...
  try {
    ...
    // 클라이언트 getFightPlayerIds와 동일한 table 쿼리 — 캐시 HIT 시 동일 transformer 재사용
    ...
    await cacheSet(cacheKey, data, TTL.fightPlayerIds);
    ...
    console.error("[api/fightPlayerIds]", e);

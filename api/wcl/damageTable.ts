// ============================================
// /api/wcl/damageTable — DamageDone 요약 테이블 공유 캐시
// ============================================
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const cacheKey = `damageTable:${code}:${sourceId}:${startTime}:${endTime}`;
  ...
  try {
    ...
    // 클라이언트 getDamageTable과 동일 — 캐시 HIT 시 동일 transformer 재사용
    ...
    await cacheSet(cacheKey, data, TTL.damageTable);
    ...
    console.error("[api/damageTable]", e);

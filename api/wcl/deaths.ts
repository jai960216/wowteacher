// ============================================
// /api/wcl/deaths — fight 사망 이벤트 공유 캐시
// ============================================
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const cacheKey = `deaths:${code}:${startTime}:${endTime}`;
  ...
  try {
    ...
    // 클라이언트 getDeaths와 동일 events 쿼리 — 캐시 HIT 시 동일 transformer 재사용
    ...
    await cacheSet(cacheKey, data, TTL.deaths);
